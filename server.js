...
// Sequelize를 이용한 세션 스토어 설정
const MyStore = SequelizeStore(session.Store);
const sessionStore = new MyStore({
    db: sequelize,
    tableName: 'sessions' // 세션 정보를 저장할 테이블 이름
});

// express-session 미들웨어 설정
app.use(session({
    secret: 'a_much_more_secure_secret_key_please_change_this',
    store: sessionStore, // 세션 스토어를 DB로 설정
    resave: false,
    saveUninitialized: false,
    cookie: {
        httpOnly: true,
        maxAge: 24 * 60 * 60 * 1000, // 24시간
        secure: process.env.NODE_ENV === 'production' // 배포 환경에서 https 사용 시 true로 설정
    }
}));
...
app.post('/api/register', async (req, res) => {
    ...
    if (!username || !password || password.length < 8) {
        return res.status(400).json({ message: '사용자 이름과 8자 이상의 비밀번호를 입력해주세요.' });
    }
    try {
...
app.post('/api/servers', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: '서버 추가 중 오류가 발생했습니다.' });
    }
});
...
app.delete('/api/servers/:id', async (req, res) => {
    ...
    try {
        ...
        if (server.User?.username === 'System') {
            return res.status(403).json({ error: '기본 서버는 삭제할 수 없습니다.' });
        }
        if (server.UserId !== req.session.user.id) {
            return res.status(403).json({ error: '서버를 삭제할 권한이 없습니다.' });
        }
        ...
        res.status(500).json({ error: '서버 삭제 중 오류가 발생했습니다.' });
    }
});
...
const seedInitialServers = async () => {
    try {
        ...
        if (fs.existsSync(serversJsonPath)) {
            ...
            console.log('🟡 servers.json 파일을 찾을 수 없어 시딩을 건너뜁니다.');
        }
    } catch (error) {
...
const startServer = async () => {
    try {
        ...
        console.log('✅ 세션 테이블 동기화 완료.');
        
        await sequelize.sync({ alter: true }); // alter: true 옵션으로 안전하게 테이블 구조 변경
